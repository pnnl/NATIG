...
//serverAddr = Ipv4Address();
  ...
  NS_LOG_INFO ("Ipv4L3ProtocolMIM::IsDestinationAddress >>> address: " << address << "   VictimAddress: " << victimAddr);
...
//      }
      ...
      if (address == iaddr.GetLocal ())
        {
    	  NS_LOG_INFO ("For me (destination " << address << " match)");
          return true;
        }
      if (address == iaddr.GetBroadcast ())
        {
    	  NS_LOG_INFO ("For me (interface broadcast address)");
          return true;
        }
    }
  ...
  if (address.IsMulticast ())
    {
      ...
      if (true)
        {
    	  NS_LOG_INFO ("For me (Ipv4Addr multicast address");
          return true;
        }
    }
  ...
  if (address.IsBroadcast ())
    {
	  NS_LOG_INFO ("For me (Ipv4Addr broadcast address)");
      return true;
    }
          ...
          for (uint32_t i = 0; i < GetNAddresses (j); i++)
            {
              ...
              if (address == iaddr.GetLocal ())
                {
            	  NS_LOG_INFO ("For me (destination " << address << " match) on another interface");
                  return true;
                }
              //  This is a small corner case:  match another interface's broadcast address
              if (address == iaddr.GetBroadcast ())
                {
            	  NS_LOG_INFO ("For me (interface broadcast address on another interface)");
                  return true;
                }
            }
        }
...
/*end edit
bool
Ipv4L3ProtocolMIM::IsDestinationAddressMIM (Ipv4Address address, uint32_t iif) const
{
  NS_LOG_FUNCTION (this << address << iif);
  NS_LOG_INFO ("Ipv4L3ProtocolMIM::IsDestinationAddress >>> address: " << address);
  // First check the incoming interface for a unicast address match
  for (uint32_t i = 0; i < GetNAddresses (iif); i++)
    {
      Ipv4InterfaceAddress iaddr = GetAddress (iif, i);
...
/*
  *temp2 = 5;
  NS_LOG_INFO ("Ipv4L3ProtocolMIM::Receive >>> tmp " << *temp2);
*/
  ...
  packet->RemoveHeader (ipHeader);
  NS_LOG_INFO ("Ipv4L3ProtocolMIM::Receive >>> ipheader " << ipHeader);
  ...
  for (SocketList::iterator i = m_sockets.begin (); i != m_sockets.end (); ++i)
    {
      NS_LOG_INFO ("Forwarding to raw socket");
...
{
  NS_LOG_INFO (this << ad << interfaceMask);
                      ...
                      Ptr<Ipv4Route> route)
{
  ...
  if (destination.IsBroadcast () || destination.IsLocalMulticast ())
    {
      NS_LOG_LOGIC ("Ipv4L3ProtocolMIM::Send case 1:  limited broadcast");
      ...
      for (uint32_t j = 0; j < GetNAddresses (ifaceIndex); j++)
        {
          ...
          if (destination.IsSubnetDirectedBroadcast (ifAddr.GetMask ()) &&
            ...
            {
              NS_LOG_LOGIC ("Ipv4L3ProtocolMIM::Send case 2:  subnet directed bcast to " << ifAddr.GetLocal ());
              ipHeader = BuildHeader (source, destination, protocol, packet->GetSize (), ttl, tos, mayFragment);
              Ptr<Packet> packetCopy = packet->Copy ();
              m_sendOutgoingTrace (ipHeader, packetCopy, ifaceIndex);
              CallTxTrace (ipHeader, packetCopy, m_node->GetObject<Ipv4> (), ifaceIndex);
  ...
  if (route && route->GetGateway () != Ipv4Address ())
    {
      NS_LOG_LOGIC ("Ipv4L3ProtocolMIM::Send case 3:  passed in with route");
      ipHeader = BuildHeader (source, destination, protocol, packet->GetSize (), ttl, tos, mayFragment);
  ...
  if (route && route->GetGateway () == Ipv4Address ())
    {
      ...
      // to be queried).
      NS_FATAL_ERROR ("Ipv4L3ProtocolMIM::Send case 4: This case not yet implemented");
    }
  // 5) packet is not broadcast, and route is NULL (e.g., a raw socket call)
  NS_LOG_LOGIC ("Ipv4L3ProtocolMIM::Send case 5:  passed in with no route " << destination);
  ...
  if (m_routingProtocol != 0)
    ...
    {
      NS_LOG_ERROR ("Ipv4L3ProtocolMIM::Send: m_routingProtocol == 0");
    }
  if (newRoute)
    {
                             ...
                             Ipv4Header  &ipHeader)
{
  ...
  NS_LOG_INFO("Ipv4L3ProtocolMIM::GetDestination: " << ipHeader.GetDestination() << "Source: " << sourceAddr << "victim address: " << victimAddr);
  if(ipHeader.GetDestination() == victimAddr) {
    ipHeader.SetSource(sourceAddr);
  }
  ...
  NS_LOG_INFO ("Ipv4L3ProtocolMIM::SendRealOut >>> Send via NetDevice ifIndex " << outDev->GetIfIndex () << " ipv4InterfaceIndex " << interface);

  if (route) //!route->GetGateway ().IsInitialized ())//!route->GetGateway ().IsEqual (Ipv4Address ("0.0.0.0")))
    {
  ...
  for (mapIter = ttlMap.begin (); mapIter != ttlMap.end (); mapIter++)
    {
      ...
      if (h.GetTtl () == 0)
        {
          NS_LOG_WARN ("TTL exceeded.  Drop. WX");
...
Ipv4L3ProtocolMIM::IpForward (Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header)
{
  ...
  NS_LOG_INFO ("Ipv4L3ProtocolMIM::IpForward >>> Forwarding logic for node: " << m_node->GetId ());
  // Forwarding
  ...
  if (ipHeader.GetTtl () == 0)
    {
      ...
      NS_LOG_WARN ("TTL exceeded.  Drop. X");
...
{
  //NS_LOG_INFO ("Ipv4L3ProtocolMIM::LocalDeliver >>> " << this << " packet: " << packet << " &ip: " << &ip << " iif: " << iif);
  ...
  //NS_LOG_INFO ("Ipv4L3ProtocolMIM::LocalDeliver >>> ipHeader.GetDestination() " << ipHeader.GetDestination());

  destAddr = ipHeader.GetDestination();
  sourceAddr = ipHeader.GetSource();
  //NS_LOG_INFO ("Ipv4L3ProtocolMIM::LocalDeliver >>> destAddr " << destAddr);
...
{
  NS_LOG_FUNCTION ("Ipv4L3ProtocolMIM::GetAddress >>> " << this << " interfaceIndex: " << interfaceIndex << " addressIndex: " << addressIndex);
  ...
  if (it == m_fragments.end ())
    {
      ...
      m_fragmentsTimers[key] = Simulator::Schedule (m_fragmentExpirationTimeout,
                                                    &Ipv4L3ProtocolMIM::HandleFragmentsTimeout, this,
                                                    key, ipHeader, iif);
    }
  else
    {
...
void
Ipv4L3ProtocolMIM::HandleFragmentsTimeout (std::pair<uint64_t, uint32_t> key, Ipv4Header & ipHeader, uint32_t iif)
{
