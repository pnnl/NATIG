    ...
    m_s5LinkMtu (3000)
{
  ...
  //We need the PGW to handle the IPs beyond the UEs
  //m_subPgwAddressHelper.SetBase ("172.17.0.0", "255.255.0.0");
  m_subPgwAddressHelper.SetBase("172.0.0.0", "255.0.0.0");
  ...
  //We also need the TUN device on the same subnet as the devices behind the UEs
  m_subPgwAddressHelper.Assign(tunDeviceContainer);
...
NoBackhaulEpcHelper::NoBackhaulEpcHelper (std::string ip1, std::string ip2, std::string ip3, std::string ip4, std::string ip5) 
	    : m_gtpuUdpPort (2152),  // fixed by the standard
	      m_s11LinkDataRate (DataRate ("10Gb/s")),
              m_s11LinkDelay (Seconds (0)),
	      m_s11LinkMtu (3000),
	      ...
	      m_s5LinkMtu (3000)
{
    ...
    m_x2Ipv4AddressHelper.SetBase (ip1.c_str(), "255.255.255.252");
    m_s11Ipv4AddressHelper.SetBase (ip2.c_str(), "255.255.255.252");
    m_s5Ipv4AddressHelper.SetBase (ip3.c_str(), "255.255.255.252");
    m_subPgwAddressHelper.SetBase(ip5.c_str(), "255.0.0.0");
    m_uePgwAddressHelper.SetBase (ip4.c_str(), "255.0.0.0");
    m_uePgwAddressHelper6.SetBase ("7777:f00d::", Ipv6Prefix (64));
    ...
    //std::string ip = "8.0.0.2";
    std::string delimiter = ".";
    std::string ip = ip4.substr(0, ip4.find(delimiter)) + ".0.0.2";
    m_pgwApp = CreateObject<EpcPgwApplication> (m_tunDevice, pgwS5Address, pgwS5uSocket, pgwS5cSocket, ip);
    m_pgw->AddApplication (m_pgwApp);
...
void
NoBackhaulEpcHelper::OverrideIP(std::string ip)
{
    //m_uePgwAddressHelper.NewNetwork();
    m_uePgwAddressHelper.SetBase (ip.c_str(), "255.0.0.0");
    NetDeviceContainer tunDeviceContainer;
    tunDeviceContainer.Add (m_tunDevice);
    m_uePgwAddressHelper.Assign (tunDeviceContainer);      
}
                                       ...
                                       const Ipv4Address &enb2X2Address) const
{
  ...
  uint16_t enb1CellId = enb1LteDevice->GetCellId (); //enb1CellIds.at (0);
  uint16_t enb2CellId = enb2LteDevice->GetCellId ();//enb2CellIds.at (0);
