 ...
 *
 *
 * TestFncs creates a dumbbell topology and logically splits it in
 ...
 * logical processor to another (the link between n4 and n5), FNCS messages
...
NS_LOG_COMPONENT_DEFINE ("SimpleFncs");
...
main (int argc, char *argv[])
{
  ...
  GlobalValue::Bind ("SimulatorImplementationType",
          StringValue ("ns3::FncsSimulatorImpl"));

  LogComponentEnable ("PacketSink", LOG_LEVEL_INFO);

  uint32_t systemId = Simulator::GetSystemId ();
  uint32_t systemCount = Simulator::GetSize ();
  ...
  return 0;
#else
  NS_FATAL_ERROR ("Can't use distributed simulator without FNCS compiled in");
#endif
}
