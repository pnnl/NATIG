                          ...
                          const Address &to, NetDevice::PacketType packetType)
{
  ...
  //Add monitoring agent here
  /*std::ofstream outfile;
  if(not (access( "monitor.txt", F_OK ) == 0)){
         outfile.open("monitor.txt", std::ios_base::app);
         outfile << "Timestamp : Bytes Received : From IP : Node ID : Packet UID\n";
         outfile.close();
   }

  outfile.open("monitor.txt", std::ios_base::app); // append instead of overwrite
  outfile  << Simulator::Now () <<
         " : " << packet->GetSize () <<
         " : " << device->GetNode()->GetObject<Ipv4>()->GetAddress(1,0).GetLocal() <<
         " : " << to <<
         " : " << packet->GetUid () << "\n";
  outfile.close();*/
                      ...
                      Ptr<Ipv4Route> route)
{
  ...
  if (route)
    ...
    ss << m_interfaces[0]->GetDevice()->GetNode()->GetObject<Ipv4>()->GetAddress(1,0).GetLocal();//ipHeader.GetSource();
    ...
    ss2 >> ID2;
    int sel_elem = 0;
    if( ID== 192 and initDinRoute){
        NS_LOG_UNCOND("Router found at " << ss.str() << " " << ID << " " << ID2 );
        if (map.size() == 0){
	      NS_LOG_UNCOND("The map is equal to 0 therefore i will be using the default output device");
        }else{
              NS_LOG_UNCOND("I found some NetDevices in the map");
              int NumRouter = map.size();
              std::vector<int> ind;
              for (int i = 0; i < NumRouter; i++){
	          ind.push_back(i);
              }
                  int random = rand() % ind.size();
		  sel_elem = ind[random];
		  while (sel_elem + 2 == ID2){
		       random = rand() % ind.size();
		       sel_elem = ind[random];
		  }
		  NS_LOG_UNCOND("The current IP of the output device " << ss.str());
		  NS_LOG_UNCOND("The selected ID is " << sel_elem);
		  NS_LOG_UNCOND("The IP of the select output device is " << map[sel_elem]->GetNode()->GetObject<Ipv4>()->GetAddress(1,0).GetLocal());
	}
   }
  ...
  if (destination.IsBroadcast () || destination.IsLocalMulticast ())
    {
      ...
      for (Ipv4InterfaceList::iterator ifaceIter = m_interfaces.begin ();
           ifaceIter != m_interfaces.end (); ifaceIter++, ifaceIndex++)
        {
	      ...
	      /*if (initDinRoute){
                      route->SetOutputDevice (map[sel_elem]); //
	      }else{*/
                             ...
                             Ipv4Header const &ipHeader)
{
  ...
  ss << m_node->GetObject<Ipv4>()->GetAddress(1,0).GetLocal();//ipHeader.GetSource();
  std::vector<std::string> ip;
  std::string ip_ = ss.str();
  size_t pos = 0;
  while ((pos = ip_.find(delimiter)) != std::string::npos) {
  ...
  if( ID== 192 and initDinRoute){
    ...
    if (map.size() == 0){
      ...
      //if (ID2 != 2 and ID2 != NumRouter + 2){
	int random = rand() % ind.size();
	int sel_elem = ind[random];
	while (sel_elem + 2 == ID2){
          random = rand() % ind.size();
          sel_elem = ind[random];
	}
  ...
  for (mapIter = ttlMap.begin (); mapIter != ttlMap.end (); mapIter++)
    {
      ...
      ss << GetNetDevice(interface)->GetNode()->GetObject<Ipv4>()->GetAddress(1,0).GetLocal();//ipHeader.GetSource();
      std::vector<std::string> ip;
      std::string ip_ = ss.str();
      size_t pos = 0;
      while ((pos = ip_.find(delimiter)) != std::string::npos) {
      ...
      if( ID== 192 and initDinRoute){
		  ...
		  NS_LOG_UNCOND("The selected ID is " << sel_elem);
		  NS_LOG_UNCOND("The IP of the select output device is " << map[sel_elem]->GetNode()->GetObject<Ipv4>()->GetAddress(1,0).GetLocal());
		  rtentry->SetOutputDevice(map[sel_elem]);
	     //}
	}
      }else{*/
...
Ipv4L3Protocol::IpForward (Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header)
{
  ...
  //Adding the action checker
  /*
  if (ipHeader.GetTtl () == 1){
    Ptr<Ipv4> current_out_ipv4 = rtentry->GetOutputDevice ()->GetNode()->GetObject<Ipv4>();
    Ptr<NetDevice> selDev = rtentry->GetOutputDevice ();
    std::stringstream ss1;
    ss1 << current_out_ipv4->GetAddress(1,0).GetLocal();
    for (int i = 0; i < m_node->GetNDevices(); i++){
       Ptr<Ipv4> tempIpv4 = m_node->GetObject<Ipv4>();
       std::stringstream ss2;
       ss2 << tempIpv4->GetAddress(i,0).GetLocal();
       //check to find if there is a different device that we can select as an output to the traffic
       if (ss1.str().find(ss2.str()) == std::string::npos){
	  NS_LOG_UNCOND("Found a device!!!!!!!!!!!!!! ;;;;;;;;;;;");
          selDev = m_node->GetDevice(i);
       }
    }
    std::stringstream ss3;
    ss3 << selDev->GetNode()->GetObject<Ipv4>()->GetAddress(1,0).GetLocal();
    //compare the selected device to see if it's equal to the current output device
    if (ss3.str().find(ss1.str()) == std::string::npos){
      ipHeader.SetTtl (ipHeader.GetTtl () + 1);
      rtentry->SetOutputDevice(selDev);
    }
  }*/
  if (ipHeader.GetTtl () == 0)
    {
