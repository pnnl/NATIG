  ...
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ...
      dnp3Master.SetAttribute("RemoteAddress", AddressValue(ueIpIface.GetAddress(u)));//ueNode->GetObject<Ipv4>()->GetAddress(1,0).GetLocal()));//star.GetSpokeIpv4Address (i)));
      ...
      std::cout << "LLLLLLLLLLLLLLLLLLLLLLLL" << std::endl;
      Dnp3ApplicationHelperNew dnp3Outstation ("ns3::UdpSocketFactory", InetSocketAddress (ueIpIface.GetAddress(u),port)); //ueNode->GetObject<Ipv4>()->GetAddress(1,0).GetLocal(), port)); //star.GetSpokeIpv4Address (i), port));
      ...
      Ptr<Dnp3ApplicationNew> slave = dnp3Outstation.Install (ueNode, std::string(ep_name));
