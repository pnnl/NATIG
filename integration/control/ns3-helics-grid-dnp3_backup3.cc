...
main (int argc, char *argv[])
{
  ...
  for (i = 0;i < configObject["microgrid"].size();i++)
  {
    ...
    Dnp3HelicsApplicationHelper dnp3Master ("ns3::UdpSocketFactory", InetSocketAddress (hubNode.Get(0)->GetObject<Ipv4>()->GetAddress(ID,0).GetLocal(), master_port));  //star.GetHubIpv4Address(i), master_port));
    ...
    Ptr<Dnp3HelicsApplication> master = dnp3Master.Install (hubNode.Get(0), std::string(cc_name+ep_name));
    dnpMasterApp.Add(master);

    Dnp3HelicsApplicationHelper dnp3Outstation ("ns3::UdpSocketFactory", InetSocketAddress (tempnode1->GetObject<Ipv4>()->GetAddress(1,0).GetLocal(), port)); //star.GetSpokeIpv4Address (i), port));
  ...
  if (includeMIM == 1){
    for (int x = 0; x < val.size(); x++){ //std::stoi(configObject["MIM"][0]["NumberAttackers"].asString()); x++){
      ...
      //scenario 1, 2 and 3
      Dnp3HelicsApplicationHelper dnp3MIM1 ("ns3::UdpSocketFactory", InetSocketAddress (MIMNode.Get(MIM_ID-1)->GetObject<Ipv4>()->GetAddress(1,0).GetLocal(), port)); //star.GetSpokeIpv4Address(MIM_ID-1),port)); 
      ...
      Ptr<Dnp3HelicsApplication> mim = dnp3MIM1.Install (tempnode, enamestring);
