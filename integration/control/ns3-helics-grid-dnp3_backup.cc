...
main (int argc, char *argv[])
{
  ...
  if (includeMIM == 1){
    for (int x = 0; x < val.size(); x++){ //std::stoi(configObject["MIM"][0]["NumberAttackers"].asString()); x++){
      ...
      //scenario 1, 2 and 3
      Dnp3ApplicationHelper dnp3MIM1 ("ns3::UdpSocketFactory", InetSocketAddress (MIMNode.Get(MIM_ID-1)->GetObject<Ipv4>()->GetAddress(1,0).GetLocal(), port)); //star.GetSpokeIpv4Address(MIM_ID-1),port)); 
      ...
      Ptr<Dnp3Application> mim = dnp3MIM1.Install (tempnode, enamestring);
